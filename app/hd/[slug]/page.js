...
export async function generateMetadata({ params }) {
  ...
  const match = jsonData.find(
    (m) => m.title.toLowerCase().replace(/\s+/g, "-") === params.slug
  );
  ...
  return (
    <main style={{ padding: "50px 20px" }}>
      ...
      <div style={{ display: "flex", justifyContent: "center", gap: "20px" }}>
        <div>
          <img src={match.url_img_a} alt={match.title_tim_a} width="120" />
          <p>{match.title_tim_a}</p>
        </div>
        <div>
          <img src={match.url_img_b} alt={match.title_tim_b} width="120" />
